    ...
    if r.status_code == 200:
        if driver.YaId in r.json()['drivers'].keys():
            ...
            if d.status_code == 200:
                check_list = 'first_name FirstName last_name \
                    LastName phone_number Phones'.split()
            ...
            return HttpResponse("<h1>Водитель с таким id \
                не найден в Яндекс</h1>")
    else:
        return HttpResponse("<h1>Яндекс не отвечает, попробуйте \
            повторить запрос позже</h1>")
...
def all_drivers_from_base(request):
    ...
    if request.method == 'POST':
        sync_db_with_remote()
        get_qiwi_park(qiwi)
        return redirect('drivers/all-drivers/')

    return render(request, 'drivers_from_base.html', {
        'drivers': drivers,
        'count': drivers_count,
        'qiwi': qiwi
        })
...
def sync_db_with_remote():
    ...
    def del_from_db(drivers_id_from_db, drivers_id_from_remote):
        ...
        # 5956391/django-objects-filter-with-list
        ...
        for driver in drivers_id_from_db & drivers_id_from_remote:
            for ln, fn, mn, ph, bal in gen:
                if ln != r.json()['drivers'][driver].get('LastName'):
                    param_list[param_list.index(driver)+1] = \
                        r.json()['drivers'][driver].get('LastName')
                if fn != r.json()['drivers'][driver].get('FirstName'):
                    param_list[param_list.index(driver)+2] = \
                        r.json()['drivers'][driver].get('FirstName')
                if mn != r.json()['drivers'][driver].get('Surname'):
                    param_list[param_list.index(driver)+3] = \
                        r.json()['drivers'][driver].get('Surname')
                if ph != r.json()['drivers'][driver].get('Phones'):
                    param_list[param_list.index(driver)+4] = \
                        r.json()['drivers'][driver].get('Phones')
    ...
    for driver in param_list[::6]:
        if len(Driver.objects.filter(YaId=driver)) > 0:
            Driver.objects.filter(YaId=driver).last_name = \
                param_list[param_list.index(driver)+1]
            Driver.objects.filter(YaId=driver).first_name = \
                param_list[param_list.index(driver)+2]
            Driver.objects.filter(YaId=driver).middle_name = \
                param_list[param_list.index(driver)+3]
            Driver.objects.filter(YaId=driver).phone_number = \
                param_list[param_list.index(driver)+4]
            Driver.objects.filter(YaId=driver).balance = \
                param_list[param_list.index(driver)+5]
        else:
            p = Park.objects.all()
